...
import {AppsListViewKey, AppsListPreferences, AppsListViewType, AppsQuery, HealthStatusBarPreferences, services} from '../../../shared/services';
...
function loadApplications(q: AppsQuery): Observable<{applications: models.Application[]; stats: models.ApplicationListStats}> {
    const reloadStats = new BehaviorSubject<Date>(new Date());

    return from(services.applications.list({...q, fields: APP_LIST_FIELDS})).pipe(
...
const ViewPref = ({children}: {children: (pref: AppsListPreferences & {page: number; pageSize: number; search: string}) => React.ReactNode}) => (
    <ObservableQuery>
        {q => (
            <DataLoader
                load={() =>
                    combineLatest([services.viewPreferences.getPreferences().pipe(map(item => ({...item.appList, pageSize: item.pageSizes['applications-list'] || 5}))), q]).pipe(
                        map(items => {
...
function isFiltered(prefs: AppsListPreferences): boolean {
    return (
        !!prefs.projectsFilter || !!prefs.labelsFilter || !!prefs.healthFilter || !!prefs.syncFilter || !!prefs.namespacesFilter || !!prefs.clustersFilter || !!prefs.autoSyncFilter
    );
}
                                    ...
                                    <DataLoader
                                        ...
                                        load={() =>
                                            AppUtils.handlePageVisibility(() =>
                                                loadApplications({...prefsToQuery(pref, query.get('search')), appNamespace: query.get('appNamespace')})
                                            )
                                        }
                                        loadingRenderer={() => (
